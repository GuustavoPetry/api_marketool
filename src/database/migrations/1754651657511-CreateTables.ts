...
export class CreateTables1754651657511 implements MigrationInterface {
    name = 'CreateTables1754651657511'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`Operation\` (\`id\` int NOT NULL AUTO_INCREMENT, \`operationType\` enum ('COMPRA', 'VENDA') NOT NULL, \`assetTicker\` char(10) NOT NULL, \`quantity\` int NULL, \`unitPrice\` decimal(10,2) NOT NULL, \`totalPrice\` decimal(10,2) NOT NULL, \`note_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`brokerage_note\` (\`id\` int NOT NULL AUTO_INCREMENT, \`date\` date NOT NULL, \`broker\` varchar(30) NOT NULL, \`document\` varchar(20) NOT NULL, \`totalValue\` decimal(10,2) NOT NULL, \`user_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`custody\` (\`id\` int NOT NULL AUTO_INCREMENT, \`assetTicker\` char(10) NOT NULL, \`assetType\` varchar(20) NULL, \`assetSector\` varchar(30) NULL, \`quantity\` int NOT NULL, \`totalInvested\` decimal(10,2) NOT NULL, \`user_id\` int NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
